...
void PID::run(){
    ...
    while(ros::ok()){
      ...
      if(pose_d(2) > 0){ // no command
          ...
          velocity_msg.x = (k_p * error(0) + k_i * error_i(0) + k_d * error_d(0)) * k_o;
          velocity_msg.y = (k_p * error(1) + k_i * error_i(1) + k_d * error_d(1)) * k_o;
          velocity_msg.z = (k_p * error(2) + k_i * error_i(2) + k_d * error_d(2)) * k_o;
          velocity_msg.w = 0; //(k_p * error(3)) * k_o;
          velocity_publisher.publish(velocity_msg);
      }

      //cout << "[PID]: " << (ros::Time::now() - begin) << endl;
    }
}
